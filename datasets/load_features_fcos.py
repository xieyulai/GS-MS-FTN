...
def load_features_from_npy_fcos(cfg, feature_names_list, video_id, start, end, duration,
    ...
    if 'glove_features' in feature_names_list:
        try:
            stack_text_global = np.load(os.path.join(cfg.global_text_features_path, f'{video_id}.npy'))
            stack_text_global = torch.from_numpy(stack_text_global).float()
            stack_text = stack_text_global
            # if get_full_feat:
            #     stacks['orig_feat_length']['text'] = stack_text_global.shape[0]
            #     if cfg.modality == 'audio_video_text':
            ...
            if cfg.procedure == 'train_prop':
                stack_text = torch.zeros(1, 300).float()
            else:
                stack_text = None
