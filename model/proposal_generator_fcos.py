...
class TrimodalProposalGeneratorFCOS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Backbone network
        self.backbone_Av = BackBone(cfg.d_model_audio, self.C3_inplanes, self.C4_inplanes, self.C5_inplanes)
        self.backbone_Va = BackBone(cfg.d_model_video, self.C3_inplanes, self.C4_inplanes, self.C5_inplanes)
    ...
    def forward(self, src, targets, masks):
        ...
        # ２、特征融合编码
        Avt, Tav, Vat, Tva = self.encoder((A, V, T), masks)

        ## 时序语义对齐
        Avt_up = upsample(Avt, self.cfg.scale_audio)
        Vat_up = upsample(Vat, self.cfg.scale_video)

        # 3、fcos实现proposal训练
        props_Av, loss_Av, losses_Av = self.fcos_prop(self.backbone_Av, self.fpn_Av, self.head_Av, Avt_up, targets)
        props_Va, loss_Va, losses_Va = self.fcos_prop(self.backbone_Va, self.fpn_Va, self.head_Va, Vat_up, targets)
